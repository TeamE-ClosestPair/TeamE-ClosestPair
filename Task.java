...
public class Task {
    ...
    public static double solve(int start, int end) {
        ...
        if (length == 2) return dist(s[start], s[start+1]);
            ...
            int mid = (start + end) / 2;
            double d = Math.min(solve(start, mid), solve(mid, end));
            ...
            if (new_len >= 2) {
                new_lst.sort(new Comparator<int[]>() {
                    @Override
                ...
                for (int i = 0; i < new_len - 1; i++) {
                    for (int j = i + 1; j < new_len; j++) {
                        if (Math.pow((new_lst.get(i)[1] - new_lst.get(j)[1]), 2) > d ) {
                            break;
                        } else if ((new_lst.get(i)[0] < midx) && (new_lst.get(j)[0]) < midx) {
                            continue;
                        } else if ((new_lst.get(i)[0] >= midx) && (new_lst.get(j)[0]) >= midx) {
                            continue;
                        }
